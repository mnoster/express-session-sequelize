...
module.exports = (Store) => {
	class SequelizeSessionStore extends Store {
		constructor(options) {
			...
			if (options.table) {
				realOptions.table = options.table;
			}

			if (options.customColumns) {
				realOptions = options.customColumns;
			}
			...
			let sess = require(path.join(__dirname, 'models', 'Session'))

			this.Session = options.db.import(sess);
